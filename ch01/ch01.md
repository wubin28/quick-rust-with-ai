...
# 1.1 The Appeal of Rust Isn't Actually Memory Safety
...
The first 10 chapters of this book will guide you through creating fun games on the micro:bit v2 development board using Rust. Just plug the USB charging port into the breadboard, and you'll see "RUST" scroll brilliantly across this palm-sized micro:bit v2 board, **lighting up your desk all day**. That's far more engaging than running a simple string program in your laptop's terminal, only to close the lid and forget about it. Take a look at Figure 1 to see what I mean.
...
| **Operating System** | **Ubuntu 24.04.1 LTS** | **macOS Sequoia 15.1.1** | **Windows 11 Pro 23H2** |
| --- | --- | --- | --- |
| Rust | rustc: 1.82.0 (f6e511eec 2024-10-15) | (same as left) | (same as left) |

## 1.2.4 Will Rust Work?
...
✅To get started, you'll need to follow two simple steps: install the [code generation tool cargo-generate](https://github.com/cargo-generate/cargo-generate), then use it to generate code from [my GitHub template repository](https://github.com/wubin28/rusty_mb2_template_light_up_microbit_board). From there, you'll be able to customize the code to create your own program.
...
⚠️If you can't run the `cargo generate` command for any reason, don't worry. You can find the `lu1l` folder with the pre-generated code in the ch01 folder of the [github.com/wubin28/learn_rust_by_games](https://github.com/wubin28/learn_rust_by_games) repository.
...
✅Now that we understand this error, let's run the following command to install the target platform, so the compiler can perform **cross-compilation** (the process of compiling programs on Linux, macOS, or Windows to run on the micro:bit v2 board):
...
Hold on—weren't we trying to light up the LED in row 1, column 1 of the 5×5 LED matrix? Yet we see the LED in row 4, column 4 glowing instead. Let's examine the code in src/main.rs (lines starting with `//` contain explanatory comments):
...
| **Computer Operating System** | **Ubuntu 24.04.1 LTS** | **macOS Sequoia 15.1.1** | **Windows 11 Pro 23H2** |
| --- | --- | --- | --- |
