# Chapter 1: Lighting Up Your First LED and Programming Supertools
...
The Part One of this book will guide you through creating fun games on the micro:bit v2 development board (hereafter referred to as mb2) using Rust. Just plug the USB charging port into the breadboard, and you'll see "RUST" scroll brilliantly across this palm-sized mb2 board, **lighting up your desk all day**. That's far more engaging than running a simple string program in your laptop's terminal, only to close the lid and forget about it. Take a look at Figure 1 to see what I mean.

![f1-1.gif](./f1-1.gif)

Figure 1-1 Scrolling display of "RUST" on the mb2 development board
...
| Step | Description |
| --- | --- |
| 1. Hardware Setup | Prepare mb2 board and USB micro data cable |
...
| 6. Understand the Code | General Rust project management files: Cargo.toml and Cargo.lock<br>Cargo tool behavior and project build environment configuration file: .cargo/config.toml<br>Embedded development specific configuration file: Embed.toml<br>Rust source code entry file: src/main.rs |
...
Next, let me introduce an exciting device—the **mb2 development board**, as shown in Figures 1-2 and 1-3.

![f1-2.jpg](./f1-2.jpg)

Figure 1-2: The front of the mb2, showing the 5×5 LED matrix and buttons on both sides

![f1-3.jpg](./f1-3.jpg)

Figure 1-3 The back view of the mb2, featuring the product name, integrated chips, reset button, and USB micro data cable
...
Five years later came the mb2. This "magic box"—smaller than a credit card—packs impressive features: an LED matrix display, two buttons, and various environmental sensors. At its heart lies the Nordic nRF52833 chip, which combines Bluetooth connectivity with remarkable power efficiency.
...
Additionally, you'll need a standard **USB micro data cable** to connect your computer's USB port to the mb2's USB port. This data cable typically comes included when you purchase a mb2.

New mb2 development boards usually work perfectly out of the box, but it's reassuring to test them. Fortunately, testing is simple since the code is already prepared—no writing required.

✅How to verify that your newly purchased micro:bit works properly?

You can visit the [micro:bit official online editor](https://python.microbit.org/v/3) to write Python code, then **flash** the program to the board with one click. You can verify the board is working properly by making the mb2's LED lights blink, display a heart pattern, and show a "Hello" greeting.
...
| **bash on Ubuntu 24.04.1 LTS** | **zsh on macOS Sequoia 15.1.1** | **CMD on Windows 11 Pro 23H2** |
|--- | --- | --- |
...
| **Installing Rust on Ubuntu** | **Installing Rust on macOS** | **Installing Rust on Windows** |
| --- | --- | --- |
...
# 1.3 Lighting Up the First LED in the Top Left Corner

Now that we've completed our "Hello, world!" program in Rust, let's dive into embedded programming. Our goal is to write code that lights up the first LED in the top-left corner of the mb2's 5×5 LED matrix.

## 1.3.1 Generating the Code
...
## 1.3.2 Compiling the Code

✅If you want to write a standard program for your computer instead of an embedded program for the mb2 board, you can compile your code right after generating it. Just use this command to convert your code into a format your operating system can understand:

```bash
cargo build
```

⚠️When compiling embedded Rust code for the mb2, we'll hit our first obstacle. You'll see this error message (note: the `$`  at the start of the first line is just the command prompt—don't type it when entering commands):
...
This configuration tells cargo to use `thumbv7em-none-eabihf` as the global compilation target. All build operations (including cargo build and cargo run) will target this platform. The Rust compiler will generate code for the ARM Cortex-M4 processor core ([located inside the Nordic nRF52833 microcontroller chip on the mb2 board's left back](https://tech.microbit.org/hardware/), as shown in Figure 1-4). This allows your program to run directly on the mb2 hardware instead of a desktop computer. The compiler stores the resulting binary files in the target/thumbv7em-none-eabihf/ folder.

![f1-4.png](./f1-4.png)

Figure 1-4 The ARM Cortex-M4 processor core is located inside the Nordic nRF52833 microcontroller chip on the back left of the mb2 board

🔎How do the mb2 board, Nordic nRF52833 microcontroller (MCU), and ARM Cortex-M4 processor core work together?

The mb2 is an educational microcontroller board powered by the Nordic nRF52833 chip. Think of the nRF52833 as the board's "brain."
...
These components fit together like nested boxes: the mb2 board houses the nRF52833 chip, which in turn contains the ARM Cortex-M4 core. This architecture provides both robust computing power and modern wireless capabilities.
...
✅Now that we understand this error, let's install the target platform. This will enable the compiler to perform **cross-compilation** (compiling programs on Linux, macOS, or Windows that will run on the mb2 board. Since all compilation in Part 1 targets the mb2 board, we'll simply refer to this as "compilation" from now on):
...
This command first builds the program (like `cargo build`) and then runs it. For the mb2, running programs requires an additional step—we need debugging tools (like J-Link or the probe-rs cargo-embed plugin) to flash the program into the mb2's memory.

⚠️When attempting to flash the program to the mb2 board, we get this error:
...
`probe-rs run` is a command that flashes compiled programs to embedded devices like mb2 and runs them. It serves as an open-source alternative to proprietary tools like J-Link.
...
nRF52833_xxAA identifies the specific MCU model. The mb2's core processor is the Nordic nRF52833 microcontroller. The suffix _xxAA indicates:
...
After installing probe-rs, try flashing the program to the mb2 board again (Note: Windows users may encounter an error, explained below):
...
🔎What is CMSIS-DAP v1? Why does Windows 11 see the mb2 as two devices?
...
When connecting a mb2 to Windows 11, the system detects two devices because of the board's dual-mode design: a main debug interface (BBC micro:bit CMSIS-DAP) and a legacy interface (CMSIS-DAP v1) for compatibility. Though both interfaces share the same device ID, Windows treats them as separate devices.
...
Table 1-3 File Management Tools for Different Operating Systems
...
panic-halt = "0.2.0"
# BBC mb2 development board support package
microbit-v2 = "0.15.0"
...
In this project, the configuration file demonstrates these capabilities well. It defines the project "lu1l" using Rust 2021 edition and includes essential embedded development dependencies like cortex-m-rt and embedded-hal. It also configures BBC mb2 board support and crash handling mechanisms.
...
🧠 What's the purpose of listing dependencies under `[dependencies]` in Cargo.toml? What happens if we remove them? Let's see how these dependencies connect to the code in [main.rs](http://main.rs).

💡 Here's how each dependency maps to our code:

- `cortex-m-rt` → `use cortex_m_rt::entry` and `#[entry]` attribute
- `panic-halt` → `use panic_halt as _`
- `microbit-v2` → `use microbit::board::Board`
- `embedded-hal` → `use embedded_hal::digital::OutputPin`

🧠 Why do we need both microbit-v2 and embedded-hal dependencies to control the LED? How do they complement each other?

💡 Think of `microbit-v2` as a skilled butler managing our development board. But even the best butler needs a clear set of rules for operating devices like LEDs. These rules need to work consistently across different hardware platforms.

This is where `embedded-hal` comes in—it's like a universal instruction manual that defines standard interfaces (such as the `OutputPin` trait). It provides the butler with clear guidelines: "Here's exactly how to turn devices on and off." This standardization ensures consistent operation across different hardware systems.

The two work in harmony: `microbit-v2`, our butler, controls the hardware by following `embedded-hal`'s standard methods. When we want to light up an LED, the butler follows the `OutputPin` trait's specifications.

But lighting up an LED requires more than just a butler and manual—we need a whole team. The "team leader" `nrf52833-hal` focuses on what needs to be done, implementing `embedded-hal` for the nRF52833 platform, while the "technical specialist" `nrf52833-pac` knows exactly how to do it.
