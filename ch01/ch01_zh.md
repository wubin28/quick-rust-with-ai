...
本书第一篇将带领你在micro:bit v2开发板（以后简称mb2）上用Rust玩各种有趣的游戏。只要连上插线板上的USB充电口，耀眼的"RUST"就能在这块巴掌大小的mb2开发板上滚动显示，**整天闪耀在桌子上**。这比在笔记本电脑终端里运行一个简单的字符串程序，然后合上屏幕就忘记要有趣得多吧？不妨看看图1体会一下。

![f1-1.gif](./f1-1.gif)

图1-1 用Rust在mb2开发板上滚动显示“RUST“
...
## 1.2.3 点亮第一个LED灯极简路线图

| 步骤 | 描述 |
| --- | --- |
| 1. 准备硬件 | 准备mb2开发板及USB micro数据线 |
| 2. 安装软件 | 安装Rust |
| 3. 生成代码 | 用`cargo generate`生成点亮LED灯代码 |
| 4. 交叉编译 | 安装目标平台：`rustup target add thumbv7em-none-eabihf`<br>交叉编译：`cargo build`<br> |
| 5. 烧录运行 | 安装烧录工具probe-rs<br>烧录运行：`cargo run` |
| 6. 解读代码 | 通用Rust项目包管理文件：Cargo.toml和Cargo.lock<br>Cargo工具行为和项目构建环境配置文件：.cargo/config.toml<br>嵌入式开发特定配置文件：Embed.toml<br>Rust源代码入口文件：src/main.rs |
...
接下来，让我介绍一个有趣的设备——**mb2开发板**，如图1-2和1-3所示。

![f1-2.jpg](./f1-2.jpg)

图1-2 mb2正面展示了5×5的LED点阵和两侧的按钮

![f1-3.jpg](./f1-3.jpg)

图1-3 mb2背面展示了产品名称、几块芯片和复位按钮，图中还有一根USB micro数据线
...
五年后，升级版mb2问世了。这个不足名片大小的"魔法盒子"装载了丰富的功能：一块LED点阵显示屏，两个按键，以及多种环境传感器。最引人注目的是它的核心——Nordic nRF52833芯片，不仅支持蓝牙通信，还具有出色的能效比。
...
此外，你还需要一根普通的**USB micro数据线**，用于连接计算机USB接口和mb2的USB接口。购买mb2时通常会附赠这根数据线。

新购买的mb2开发板通常都能正常工作，不过做个测试会更让人安心。幸运的是，测试过程非常简单，因为测试代码已经准备好了，无需自己编写。

✅如何验证新购买的micro:bit能正常工作？

可以访问[micro:bit官方在线编辑器](https://python.microbit.org/v/3)编写Python代码，然后一键将程序**烧录（flash）**到开发板。通过让mb2的LED灯闪烁、显示爱心图案并显示"Hello"问候语，就能验证开发板是否正常工作。
...
完成了Rust版的"Hello, world!"程序后，让我们正式踏入Rust嵌入式编程的世界——我们将编写一段Rust代码，点亮mb2正面5×5 LED点阵中左上角的第一个LED灯。
...
✅如果你只是想编写在自己电脑上运行的普通程序，而不是为mb2开发板编写嵌入式程序，那么生成代码后，可以直接用以下命令来编译（这会将你的代码转换成操作系统能理解的格式）：

```bash
cargo build
```

⚠️当我们运行上述编译命令来编译针对mb2的嵌入式Rust代码时，会遇到第一个障碍。你将看到以下错误信息（注意：下面第一行开头的`$` 是命令行提示符，用于区分命令和输出，输入命令时不要包含它）：
...
上面这两行配置指定了全局编译目标为 `thumbv7em-none-eabihf`。这告诉 cargo，所有构建操作（包括 cargo build 和 cargo run）都将针对这个目标平台进行。Rust 编译器会生成适配 ARM Cortex-M4 处理器内核的代码（[该内核位于 mb2 开发板背面左侧的 Nordic nRF52833 微控制器芯片内](https://tech.microbit.org/hardware/)，如图1-4所示），让程序直接运行在 mb2 的硬件上，而不是生成面向常规桌面平台的代码。最终生成的二进制文件会存放在 target/thumbv7em-none-eabihf/ 文件夹中。

![f1-4.png](./f1-4.png)

图1-4 ARM Cortex-M4 处理器内核位于 mb2 开发板背面左侧的 Nordic nRF52833 微控制器芯片内

🔎mb2 开发板、Nordic nRF52833 微控制器（MCU，microcontroller unit）和 ARM Cortex-M4 处理器内核是如何组合在一起的？

mb2 是一款教育用微控制器开发板，其核心是 Nordic nRF52833 微控制器芯片。可以把Nordic nRF52833 微控制器比作mb2开发板的“大脑”。
...
这三者形成了一个层层嵌套的结构：mb2 开发板采用 Nordic nRF52833 作为主控芯片，而 nRF52833 内部则使用 ARM Cortex-M4 作为处理器核心。这种设计使 mb2 不仅具备强大的计算能力，还拥有现代化的无线通信功能。
...
✅理解了这个错误后，让我们运行以下命令来安装目标平台，使编译器能进行**交叉编译**（cross-compilation，即在Linux、macOS或Windows上编译程序，让它能在mb2开发板上运行。由于第一篇中的编译都针对mb2开发板，为简洁起见，本篇后文提到的编译均指交叉编译）：
...
上面这个运行代码的命令会先"构建"程序（等同于 `cargo build`），然后"运行"它。对于 mb2，运行程序比在普通计算机上更复杂，因为需要通过调试工具（如 J-Link 或 probe-rs 的 cargo-embed 插件）将生成的程序文件烧录（flash）到 mb2 的存储器中。

⚠️果然不出所料，当要向mb2开发板烧录程序时，下面的错误信息表示我们踩到了新坑：
...
`probe-rs run` 是由 probe-rs 工具提供的命令，用于将编译好的程序烧录到嵌入式设备（如 mb2）的存储器中并运行程序。它可以替代专有工具（如 J-Link）来烧录和调试程序。
...
nRF52833_xxAA 是目标MCU的具体型号。前面讲过，mb2开发板的“大脑”就是Nordic nRF52833 微控制器。后缀 _xxAA通常用于指定芯片的封装和版本：
...
安装完probe-rs工具后，再次尝试运行下面的命令往mb2开发板上烧录程序（注意：在Windows上运行可能会报错，见后文）：
...
🔎CMSIS-DAP v1是什么？为何插在windows 11上的mb2会被识别为两个设备?
...
当你将mb2插入Windows 11电脑时，系统会显示两个设备。这是因为mb2采用了双模式设计：一个是主调试接口（BBC micro:bit CMSIS-DAP），另一个是为兼容旧系统的接口（CMSIS-DAP v1）。尽管这两个接口的设备ID相同，Windows仍将它们识别为不同的设备。
...
以本项目为例，配置文件指定了项目名称"lu1l"和Rust 2021版本，并引入了嵌入式开发所需的依赖包，如cortex-m-rt和embedded-hal。它还特别为BBC mb2开发板提供支持，并配置了多种崩溃处理机制。
